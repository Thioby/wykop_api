...
class GetProfileAddedLinks {
  final ApiClient _client;
  final LinkResponseToLinkDtoMapper _mapper;

  GetProfileAddedLinks(this._client, this._mapper);

  Future<List<LinkDto>> execute({int page = 1, @required String username}) async {
    var items = await _client.request('profiles', 'added', api: [username], named: {'page': page.toString()});
