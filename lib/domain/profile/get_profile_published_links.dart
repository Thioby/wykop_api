...
class GetProfilePublishedLinks {
  final ApiClient _client;
  final LinkResponseToLinkDtoMapper _mapper;

  GetProfilePublishedLinks(this._client, this._mapper);

  Future<List<LinkDto>> execute({int page = 1, @required String username}) async {
    var items = await _client.request('profiles', 'published', api: [username], named: {'page': page.toString()});
