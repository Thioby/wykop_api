...
class GetProfileCommentsForLink {
  final ApiClient _client;
  final LinkCommentResponseToLinkCommentDtoMapper _mapper;

  GetProfileCommentsForLink(this._client, this._mapper);

  Future<List<LinkCommentDto>> execute({int page = 1, @required String username}) async {
    var items = await _client.request('profiles', 'comments', api: [username], named: {'page': page.toString()});
